...
$('body').on('click', '.left_menu', function(event) {
    if (currentLeftSubmenu !== $(this).attr("data-menu")) {
        ...
        window.parent.postMessage({ action: "vehshop.changeTab", data: 0 }, '*');
        clearAll();
    }
});
...
initData = function(eventData) {
    ...
    let classics = eventData.classics;
    ...
    $("#changeClassics").attr("max", classics).val(0);
...
clearAll = function() {
    ...
    $("#value").attr("value", 255);
    $('#value').trigger('change');
    $("#valueOutput").text("255");
...
setter = function(event, setting, send_value) {
    ...
    switch (event) {
        case 'changeHue':
            ...
            saturation.style.backgroundImage = "linear-gradient(to right, rgba(255, 255, 255, 255), rgba(" + rgb.r + "," + rgb.g + "," + rgb.b + ", 255)";
            value.style.backgroundImage = "linear-gradient(to right, rgba(0, 0, 0, 255), rgba(" + rgb.r + "," + rgb.g + "," + rgb.b + ", 255))";
        ...
        case 'changeClassics':
            ...
            window.parent.postMessage({ action: "vehshop.changeClassics", data: { value: send_value } }, '*');
            break;
        case 'changeSuper':
            if (send_value.indexOf('.') != -1) {
                vehData[setting] = parseFloat(send_value);
